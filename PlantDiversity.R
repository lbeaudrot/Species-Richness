...
test2 <- test[-na.omit(match(ExcludePlots, test$plot)),]
test.site <- substr(test2$plot,4,6)
test2 <- cbind(test2, test.site)
test2 <- subset(test2, test.site!="CSN" & test.site!="NAK")
...
par(mar=c(5,5,1,1))
plot( meanSD[,2], meanD[,2], pch=19, xlab="Stems per hectare", ylab="Mean DBH (cm)", las=1, cex=1.5, cex.lab=1.7)
