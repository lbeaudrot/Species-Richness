...
set.panel(1,3)
par(mar=c(5, 5, 1, 1))
plot(model.data$V.Cstorage2/1000, Mdata$CT.median, las=1, ylab="Species Richness", xlab="", bty="n", xlim=c(100, 250), ylim=c(10,50), cex.lab=1.2, pch=c(15,16,17)[unclass(as.factor(Mdata$Continent))], col=c("blue","green3","red")[unclass(as.factor(Mdata$Continent))], cex=1.3)
legend("bottomleft", expression(R^2* " < 0.001, df=12, p=0.77"), cex=1, bty="n")
plot(model.data$V.Cstorage2/1000, Mdata$Shannon.Index, las=1, ylab="Taxonomic Diversity", xlab="", bty="n", xlim=c(100, 250), ylim=c(2.2, 3.4), cex.lab=1.2, pch=c(15,16,17)[unclass(as.factor(Mdata$Continent))], col=c("blue","green3","red")[unclass(as.factor(Mdata$Continent))], cex=1.3)
legend("bottomleft", expression(R^2* " = 0.03, df=12, p=0.56"), cex=1, bty="n")
plot(model.data$V.Cstorage2/1000, Mdata$CT.FDisMedian, las=1, ylab="Functional Diversity", xlab="", bty="n", xlim=c(100, 250), ylim=c(0.24,0.32), cex.lab=1.2, pch=c(15,16,17)[unclass(as.factor(Mdata$Continent))], col=c("blue","green3","red")[unclass(as.factor(Mdata$Continent))], cex=1.3)
legend("bottomleft", expression(R^2* " = 0.05, df=12, p=0.22"), cex=1, bty="n")
legend("topright", legend=c("Asia", "Americas", "Africa"), col=c("red", "green3", "blue"), pch=c(17,16,15), border="n", box.col="transparent", cex=1)
